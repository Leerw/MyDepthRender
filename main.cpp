...
pcl::PointCloud<pcl::PointXYZ>::Ptr readPCD(string filename){
	...
	std::cout << "Loaded " << filename << " " << cloud->points.size() << " points" << std::endl;
	...
	float scale_x{max_x / radius}, scale_y{max_y / radius}, scale_z{max_z / radius};
...
void display() {
	...
	cv::Mat img(glutGet(GLUT_WINDOW_HEIGHT), glutGet(GLUT_WINDOW_WIDTH), CV_32FC3);// output depth image
